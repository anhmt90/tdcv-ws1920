...
rerun_ransac = 0;
ransac_files = ["Correct_Orientations.mat","Correct_Locations.mat","Best_Inliers.mat"];
all_files_exist = 1;
for i=1:size(ransac_files,2)
    all_files_exist = all_files_exist && isfile(ransac_files(i));
end

if all_files_exist && ~rerun_ransac
    for i=1:size(ransac_files,2)
        load(ransac_files(i));
    end
else
